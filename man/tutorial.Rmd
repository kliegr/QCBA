...
acc_qcba_no_att_pr <- CBARuleModelAccuracy(predictionQCBA_NOATTPR, y_true)
condition_count_qcba_no_att_pr<-rmQCBA_NOATTPR@rules$condition_count
rule_count_qcba_no_att_pr<-rmQCBA_NOATTPR@ruleCount
```
...
acc <- CBARuleModelAccuracy(predictionQCBA, y_true)
condition_count_qcba_no_att_pr<-rmQCBA@rules$condition_count
avg_length_qcba <-sum(rmQCBA@rules$condition_count)/rmQCBA@ruleCount
```
#### Compare the three models
```{r message=FALSE}
  print(paste("CPAR: Number of rules: ",length(rmBASE$rules),", total conditions :",round(sum(rmBASE$rules@lhs@data),2), ", accuracy on test data: ",round(CBARuleModelAccuracy(predictionBASE, y_true),2)))
  print(paste("QCBA (NO ATT PR.): Number of rules: ",nrow(rmQCBA_NOATTPR@rules),", average number of conditions per rule :",rmQCBA_NOATTPR@rules$condition_count, ", accuracy on test data: ",round(CBARuleModelAccuracy(predictionQCBA_NOATTPR, y_true),2)))
  print(paste("QCBA (ATT PR.): Number of rules: ",nrow(rmQCBA@rules),", average number of conditions per rule :",rmQCBA@rules$condition_count, ", accuracy on test data: ",round(CBARuleModelAccuracy(predictionQCBA, y_true),2)))
```
