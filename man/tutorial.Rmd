...
```{r results='hide', message=FALSE}
# recreate the CBA model with default rule pruning disabled
...
rmqCBA <- qcba(cbaRuleModel=rmCBA,datadf=data_raw, extendType="numericOnly",trim_literal_boundaries = trim_literal_boundaries, postpruning = "cba", defaultRuleOverlapPruning="transactionBased",  createHistorySlot=TRUE,loglevel = "WARNING")
print(rmqCBA@rules)
```
...
rmCBAiris <- cba(trainFold, classAtt="Species", rulelearning_options = rulelearning_options,pruning_options=list(default_rule_pruning=FALSE))
rmqCBAiris <- qcba(cbaRuleModel=rmCBAiris,datadf=trainFold,extendType="noExtend", postpruning="none", defaultRuleOverlapPruning="noPruning",attributePruning = FALSE)
...
print(paste("avg rule length of CBA model:",avgRuleLengthCBA))
...
print(paste("avg rule length of QCBA model without optimizations:",avgRuleLengthQCBA))

```
This shows that QCBA leaves the CBA model untouched in terms of the number of literals (QCBA only refits literal ranges).
Now let's us build the same QCBA model with attribute pruning on:

```{r message=FALSE}
rmqCBAiris <- qcba(cbaRuleModel=rmCBAiris,datadf=trainFold, attributePruning = TRUE, extendType="noExtend", postpruning="none", defaultRuleOverlapPruning="noPruning")
print(rmqCBAiris@rules)
```
What we can observe is that 
1) the confidence of all the rules remained the same (it can also increase).
1) the accuracy of the complete classifier stays the same:
...
print(acc)
```
2) The average number of literals dropped by 60%:
```{r message=FALSE}
avgRuleLengthQCBA <- (sum(unlist(lapply(rmqCBAiris@rules[1],str_count,pattern=",")))+
                              # assuming the last rule has antecedent length zero - not counting its length
                              nrow(rmqCBAiris@rules)-1)/nrow(rmqCBAiris@rules)
...
####   Debugging qCBA execution in Java  ( experimental rule mixture)
```{r}
annotate =TRUE
