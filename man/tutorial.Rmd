...
```
As we can notice from the picture above, Rule 2 is actually not needed. It classifies a certain region as blue. However, if the rule was not there, the same region would be classified as blue by the default rule. Rule 2 was not removed by the *default rule pruning* in CBA, because of the way this pruning works: the default rule replaces all rules below (as sorted according to  confidence, support, length) below the current rule. Such pruning would remove rules 3 and 4, which would impair the overall quality of the classifier. To get rid of rules such as #2 QCBA contains a novel procedure, which we call *default rule overlap* pruning.
...
interestingRuleAsText <-  paste(paste(unlist(r$lhs[1]),collapse=''),paste(unlist(r$rhs[1]),collapse=''),sep=" => ")
